...
export default function Navbar() {
  ...
  // Close the expanded island when clicking outside
  useEffect(() => {
  ...
  // Focus search input when island expands
  useEffect(() => {
    if (isExpanded) {
          ...
          {!isExpanded ? (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <Link href="/" className="flex items-center gap-2 font-serif text-lg sm:text-xl font-bold tracking-tight text-white hover:opacity-85 transition-opacity">
              ...
              <div className="hidden sm:flex items-center space-x-6 mx-4">
                <Link href="/scholarships" className="text-xs font-semibold text-white/80 hover:text-white transition-colors">Scholarships</Link>
                
                {/* Provider Hover Dropdown */}
                <div 
                  className="relative"
                        ...
                        >
                          {providers.map((p) => (
                            <Link
                              ...
                              className="block px-3 py-2 rounded-xl text-[11px] text-white/85 hover:text-white hover:bg-white/10 transition-colors"
                            >
                              {p.flag} {p.name}
                            </Link>
                          ))}
                ...
                <Link href="/about" className="text-xs font-semibold text-white/80 hover:text-white transition-colors">About</Link>
              </div>
            ...
            >
              ...
              <div className="flex items-center justify-between pb-4 border-b border-white/15 mb-6">
                <Link 
                  ...
                  className="flex items-center gap-2.5 font-serif text-xl sm:text-2xl font-bold tracking-tight text-white hover:opacity-85 transition-opacity"
                >
                ...
                <button
                  onClick={() => setIsExpanded(false)}
                  className="p-2 hover:bg-white/10 rounded-full transition-colors text-white/85 hover:text-white cursor-pointer"
                  aria-label="Close"
                >
                  <X className="h-5 w-5" />
                </button>
              ...
              <form onSubmit={handleSearchSubmit} className="relative w-full mb-6">
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 focus:border-brand-accent/50 focus:bg-white/10 outline-none rounded-2xl py-3 px-5 pr-12 text-sm text-white placeholder-white/40 transition-all"
                />
                <button
                  type="submit"
                  className="absolute right-3 top-1/2 -translate-y-1/2 p-2 hover:bg-white/10 rounded-xl transition-colors text-white/60 hover:text-white cursor-pointer"
                >
                    ...
                    {providers.map((p) => (
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-white group-hover/item:text-brand-accent transition-colors truncate">
                            {p.name}
                          </p>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-2">
                      <Link
                        href="/scholarships"
                        onClick={() => setIsExpanded(false)}
                        className="flex items-center justify-between p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors text-xs font-semibold cursor-pointer"
                      >
