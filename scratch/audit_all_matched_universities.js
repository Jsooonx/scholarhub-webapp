...
const issues = [];
const specificUniKeywords = [
  'university', 'universiti', 'universidad', 'universiteit', 'universitat', 'school', 'college', 'institute', 'institut', 'polytechnic'
];
...
data.scholarships.forEach((s) => {
  ...
  // Check if this looks like a specific university scholarship
  const isSpecificUni = specificUniKeywords.some(kw => nameLower.includes(kw) || providerLower.includes(kw))
    && !nameLower.includes('ministry') 
    && !nameLower.includes('government') 
    && !nameLower.includes('fellowship')
    && !providerLower.includes('ministry')
    && !providerLower.includes('government')
    && !providerLower.includes('education above all')
    && !nameLower.includes('erasmus')
    && !nameLower.includes('stipendium hungaricum');
  ...
  if (isSpecificUni) {
    if (matches.length === 0) {
      issues.push({
        type: 'Zero Matches (Fallback Triggered)',
        scholarship: s.name,
        provider: s.provider,
        country: s.country,
        fallbackType: providerGroup(s.provider)
      });
    } else {
      // Check if matched logos actually match the provider/name or if there's a mismatch
      // e.g. provider has "City University of Hong Kong" but matches "University of Hong Kong"
      const namesMatched = matches.map(m => m.name.toLowerCase());
      
      // Let's see if we matched something that is NOT the provider's university
      const providerUniMatch = universities.find(u => providerLower.includes(u.name.toLowerCase().split(' (')[0]));
      if (providerUniMatch) {
        const expectedName = providerUniMatch.name.toLowerCase();
        if (!namesMatched.includes(expectedName)) {
          issues.push({
            ...
            expected: providerUniMatch.name
          });
        }
      }
    }
...
console.log(`Audited ${data.scholarships.length} scholarships.`);
